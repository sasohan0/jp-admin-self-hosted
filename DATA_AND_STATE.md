...
| `syncDiscordRoster` | `roster.js` | Captures every current non-bot, non-supervisor Discord member in private Roster Review, then rebuilds active `Bot_Map` only from verified profiles with a real non-Discord-domain email, Sheet-backed full name, and valid 8–15 digit phone. It discovers OAuth intake and safe legacy identity/contact tabs by headers, trusts durable Discord IDs, and never auto-links a display name alone. Unresolved members receive a private profile request; no synthetic email or blank phone enters tracking. Old synthetic rows remain historical/migratable but inactive. |
